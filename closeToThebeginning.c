// By: Kianoush Ranjbar
// comments: sends with seqNumber to the correct port, and ACKs back, now i have to handle the duplicate ACK
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  while (ptr !=NULL){
    ...
    // the message was for me and I received it, but now i must send back an ack, to the sender
     		  ...
     		  printf("Lets see if it works111:%s\n\n", new_bufferReceived);	
    ...
    if (msg_ptr != NULL) {
        ...
        if (end_quote_ptr != NULL) {
            int len = end_quote_ptr - msg_ptr + 1; // calculate length of key and value to remove
    ...
    // add ACK field 
 		  ...
 		  printf("Lets see if it works222:%s\n\n", new_bufferReceived);	
    ...
    // now send the ACK back to the sender
     		  printf("Lets see if it works333:%s\n\n", new_bufferReceived);	
